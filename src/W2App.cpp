...
DWORD __stdcall W2App::hookInitializeW2App(DWORD DD_Game_a2, DWORD DD_Display_a3, DWORD DS_Sound_a4, DWORD DD_Keyboard_a5, DWORD DD_Mouse_a6, DWORD WAV_CDrom_a7, DWORD WS_GameNet_a8) {
	...
	Diagnostics::log("InitializeW2App: ddGame=0x%X ddDisplay=0x%X wrapper=0x%X", addrDDGame, addrDDDisplay, addrW2Wrapper);
...
DWORD __stdcall W2App::hookConstructGameGlobal(DWORD ddgame) {
	...
	Diagnostics::log("ConstructGameGlobal: ddGame=0x%X gameGlobal=0x%X", ddgame, addrGameGlobal);
	return ret;
}

DWORD (__fastcall *origDestroyGameGlobal)(int This, int EDX);
DWORD __fastcall W2App::hookDestroyGameGlobal(int This, int EDX) {
	Diagnostics::log("DestroyGameGlobal: this=0x%X", This);
	GameTextRenderer::reset();
...
void W2App::install() {
	...
	Diagnostics::log("W2App install: init=0x%X constructGlobal=0x%X destroyGlobal=0x%X ddMain=0x%X",
		addrInitializeW2App, addrConstructGameGlobal, addrDestroyGameGlobal, addrDDMain);

	_HookDefault(InitializeW2App);
	_HookDefault(ConstructGameGlobal);
	_HookDefault(DestroyGameGlobal);
	TrackTimeOverlay::install();
}
...
DWORD W2App::getAddrTurnGameObject() {
	return addrDDGame ? *(DWORD*)(addrDDGame + 0x8) : 0;
}
